...
//! Benchmark-selectable bounded mode; immutable while initialized.
static size_t _rpmalloc_global_cache_multiplier = GLOBAL_CACHE_MULTIPLIER;
//! Maximum one-span entries retained by each thread heap.
static size_t _rpmalloc_thread_span_cache_limit = MAX_THREAD_SPAN_CACHE;
//! Main thread ID
...
_rpmalloc_global_cache_insert_spans(span_t** span, size_t span_count, size_t count) {
	if (!_rpmalloc_global_cache_multiplier) {
		for (size_t ispan = 0; ispan < count; ++ispan)
			_rpmalloc_span_unmap(span[ispan]);
		return;
	}
	const size_t cache_limit = (span_count == 1) ?
		_rpmalloc_global_cache_multiplier * MAX_THREAD_SPAN_CACHE :
		_rpmalloc_global_cache_multiplier * (MAX_THREAD_SPAN_LARGE_CACHE - (span_count >> 1));
...
_rpmalloc_heap_cache_insert(heap_t* heap, span_t* span) {
	...
	if (span_count == 1) {
		...
		if (!_rpmalloc_thread_span_cache_limit) {
#if ENABLE_GLOBAL_CACHE
			_rpmalloc_global_cache_insert_spans(&span, 1, 1);
#else
			_rpmalloc_span_unmap(span);
#endif
			return;
		...
		if (span_cache->count == _rpmalloc_thread_span_cache_limit) {
			const size_t transfer_count =
				(_rpmalloc_thread_span_cache_limit < THREAD_SPAN_CACHE_TRANSFER) ?
				_rpmalloc_thread_span_cache_limit : THREAD_SPAN_CACHE_TRANSFER;
			const size_t remain_count =
				_rpmalloc_thread_span_cache_limit - transfer_count;
#if ENABLE_GLOBAL_CACHE
			_rpmalloc_stat_add64(&heap->thread_to_global, transfer_count * _memory_span_size);
...
_rpmalloc_heap_global_cache_extract(heap_t* heap, size_t span_count) {
	...
	if (span_count == 1) {
		span_cache = &heap->span_cache;
		wanted_count = _rpmalloc_thread_span_cache_limit == 0 ? 1 :
			((_rpmalloc_thread_span_cache_limit < THREAD_SPAN_CACHE_TRANSFER) ?
			 _rpmalloc_thread_span_cache_limit : THREAD_SPAN_CACHE_TRANSFER);
	} else {
		span_cache = (span_cache_t*)(heap->span_large_cache + (span_count - 2));
...
int
rpmalloc_set_global_cache_enabled(int enabled) {
	if (_rpmalloc_initialized)
		return -1;
	_rpmalloc_global_cache_multiplier = enabled ? GLOBAL_CACHE_MULTIPLIER : 0;
	return 0;
}

int
rpmalloc_set_global_cache_multiplier(size_t multiplier) {
	if (_rpmalloc_initialized || multiplier > GLOBAL_CACHE_MULTIPLIER)
		return -1;
	_rpmalloc_global_cache_multiplier = multiplier;
	return 0;
}

int
rpmalloc_set_thread_span_cache_limit(size_t span_count) {
	if (_rpmalloc_initialized || span_count > MAX_THREAD_SPAN_CACHE)
		return -1;
	_rpmalloc_thread_span_cache_limit = span_count;
	return 0;
}
